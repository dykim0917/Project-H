'use strict';
const $=id=>document.getElementById(id),P=PachinkoMark3;
...
function render(){
 ...
 const state=(world.game.demo?'체험 · ':'')+({idle:running?'PLAY':'READY',spin:'SPIN',reach:'REACH',payout:'JACKPOT +'+world.game.payout,result:world.game.win?'JACKPOT':'NEXT',unlock:'UNLOCK',opening:'OVERDRIVE',rush:'RUSH · '+world.game.chain+'연짱', 'rush-pay':world.game.chain+'연짱 · +'+world.game.payout,closing:'RUSH END'}[world.game.phase]);
...
$('reset').onclick=()=>{stopPowerHold();world=new P.World({random,power:world.power});accumulator=0;soundscape?.rolling(0);soundscape?.motion(0,false);render();draw();};
$('demo').onclick=()=>{audio();stopPowerHold();world=new P.World({random,power:world.power});world.game.demo=true;const events=[];world.game.startJackpot(events);running=false;accumulator=0;soundscape?.motion(0,false);sounds(events);render();draw();};
...
function drawRush(g){
 ...
 // The enclosed rear mechanism unfolds behind the front wheel, in a separate depth.
 for(const side of [-1,1]){c.save();c.translate(268+side*(43+45*t),377);c.rotate(side*t*.25);c.fillStyle='#987141';c.fillRect(-13,-70,26,140);path([[-9,-65],[-9,65],[9,65],[9,-65]],'#e9cd8e',2);circle(0,0,7,'#ead39e');c.restore();}
 circle(268,377,78,'#344c4d','#d5b976');circle(268,377,71,'#172e33','#9ec2b0');
 c.beginPath();c.moveTo(268,377);c.arc(268,377,65,-Math.PI/2,-Math.PI/2+Math.PI*2*P.BALANCE.rushBallChance);c.closePath();c.fillStyle='#4dbea0';c.fill();
 circle(268,377,39,'#24404a','#aa9264');star(268,377,19,'#dfc186');label('RUSH',268,408,10,'#f6dfa3');
 label(g.chain+' CHAIN',268,282,17,'#91512e');label('CONTINUE',303,347,8,'#eeffdc');
 for(const b of g.rushBalls){const u=Math.min(1,g.time/b.duration),a=-Math.PI/2+b.end-Math.PI*6*(b.resolved?0:Math.pow(1-u,3)),r=b.resolved?60:49;const x=268+Math.cos(a)*r,y=377+Math.sin(a)*r;circle(x,y,5.2,b.resolved?(b.success?'#fff4ae':'#74848a'):'#f4fcf9','#c1d4d0');}
 c.restore();
}
function draw(){
 ...
 label('별빛 회전극장',268,77,24,'#984d3b');label('S T A R L I G H T   ·   M K . I I I',268,97,8,'#8f7353');star(173,70,9,'#aa6346');star(363,70,9,'#aa6346');
 ...
 drawRush(g);
 c.save();c.translate(268,377);c.scale(Math.max(.001,1-g.transform),1);c.translate(-268,-377);
 ...
 if(g.transform<.5)label(active?g.number+' · ? · '+g.number:won?'JACKPOT':'LUCKY WHEEL',268,303,active?14:9,active?'#a13f2b':'#826044');
