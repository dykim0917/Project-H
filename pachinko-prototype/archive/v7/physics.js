...
(function (root) {
  ...
  const outerArc=arc(268,230,248,195),canopy=arc(284.5,201,149.5,95);
  const segments=points=>points.slice(1).map((p,i)=>[...points[i],...p]);
  const straightWalls = [
    [20,230,20,617],[516,230,516,617],
    [135,201,135,431],[434,201,434,431],[135,431,434,431],
  ...
  const flap={x:55,y:143,length:29,rest:-1.03};
  ...
  class World {
    ...
    launch(){const [x,y]=rail[0],power=Math.max(0,Math.min(1,this.power)),effective=launchPower(power);
      ...
      this.balls.push({id:++this.sequence,x,y,vx:0,vy:0,age:0,railDistance:0,railSpeed:850+power*350,released:false,power,
        releaseVX:60+460*effective+(this.random()-.5)*16,
        releaseVY:-(60+460*effective)*.75+(this.random()-.5)*12,trace:[]});this.shots++;}
    step(dt=STEP,firing=false){
      ...
      for(const b of this.balls){
        ...
        let impact=0;
        if(b.vx<0&&b.x<90&&b.y<160)impact=Math.max(impact,segment(b,flapSegment(this.flapAngle)));
  ...
  const api={World,STEP,WIDTH,HEIGHT,RADIUS,pins,groups,walls,rail,railPoint,railLength,stage,stageY,rightGate,launchPower,outerArc,canopy,straightWalls,flap,flapSegment};
