...
test('rush probability corresponds to three independent capsule sorting outcomes',()=>{
...
test('continuous sound layers can be silenced and impact voices stay bounded',()=>{
 const Sound=require('../mark3/sound'),param=()=>({value:0,setTargetAtTime(v){this.value=v;},setValueAtTime(v){this.value=v;},exponentialRampToValueAtTime(v){this.value=v;}});
...
test('capsules release once in order, resolve after release and stay fixed through payout',()=>{
 const g=new P.Game(()=>.1);g.chain=1;g.beginRush([]);
 const events=tick(g,4.8);assert.deepEqual(events.filter(e=>e.type==='capsule-release').map(e=>e.slot),[0,1,2]);
 assert.equal(g.chain,2);assert.ok(g.rushBalls.every(b=>b.released&&b.resolved));
 g.rushBalls.forEach((b,i)=>{assert.deepEqual(P.capsulePose(b,i,0),P.capsulePose(b,i,4));assert.equal(P.capsulePose(b,i,0).stage,'landed');});
});
test('capsules remain stored until released and paths reach the matching collector',()=>{
 const g=new P.Game(()=>.9);g.beginRush([]);
 g.rushBalls.forEach((b,i)=>{assert.equal(P.capsulePose(b,i,b.releaseAt-.01).stage,'stored');const p=P.capsulePose(b,i,b.duration);assert.ok(p.x<268);assert.equal(p.y,439);assert.equal(p.success,false);});
});
