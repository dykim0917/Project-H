...
(function (root) {
  ...
  const groups = [
    ...
    [[453,221],[478,231],[498,246]],
    [[491,274],[468,287],[451,304]],
    [[449,334],[473,345],[494,358]],
    [[494,391],[470,403],[451,418]],
    [[459,455],[486,471]],
    [[111,380]]
  ];
  groups.forEach((points,group)=>points.forEach(([x,y])=>pins.push({x,y,r:2.65,group})));
  const stage={left:176,right:391,center:268,hole:19,intake:{x:116,y:396,width:15}};
  ...
  function launchPower(power){
    ...
    if(p<=.36)return p;
    if(p<=.60)return .36+(p-.36)*(.07/.24);
    if(p<=.90)return .43+(p-.60)*(.47/.30);
    return p;
  }
  const walls = [
    [20, 50, 20, 617], [516, 45, 516, 617], [20,45,516,45],
    [135, 201, 434, 201], [135, 201, 135, 431], [434, 201, 434, 431], [135, 431, 434, 431],
    [30, 442, 197, 535], [438,440,446,514], [511,440,504,514]
  ];
  // A sampled curved launch rail keeps the guided part continuous at every frame.
  const knots = [[477,650],[411,675],[77,675],[29,628],[29,211],[34,168],[46,143],[67,132]];
  ...
  class World {
    ...
    launch(){const [x,y]=rail[0],power=Math.max(0,Math.min(1,this.power)),effective=launchPower(power);
      ...
      this.balls.push({id:++this.sequence,x,y,vx:0,vy:0,age:0,railDistance:0,railSpeed:850+power*350,released:false,power,
        releaseVX:32+520*Math.pow(effective,2.3)+(this.random()-.5)*22,
        releaseVY:-60-250*effective+(this.random()-.5)*18,trace:[]});this.shots++;}
    step(dt=STEP,firing=false){
      ...
      for(const b of this.balls){
        b.age+=dt;
        if(!b.released){b.railDistance=Math.min(railLength,b.railDistance+b.railSpeed*dt);[b.x,b.y]=railPoint(b.railDistance);if(b.railDistance===railLength){b.released=true;b.vx=b.releaseVX;b.vy=b.releaseVY;}continue;}
  ...
  const api={World,STEP,WIDTH,HEIGHT,RADIUS,pins,groups,walls,rail,railPoint,railLength,stage,stageY,rightGate,launchPower};
