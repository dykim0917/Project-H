...
(function (root) {
  ...
  const stage={left:176,right:391,center:268,hole:19,intake:{x:116,y:396,width:24}};
  ...
  class World {
    ...
    launch(){const [x,y]=rail[0],power=Math.max(0,Math.min(1,this.power));
      ...
      this.balls.push({id:++this.sequence,x,y,vx:0,vy:0,age:0,railDistance:0,railSpeed:850+power*350,released:false,power,
        releaseVX:32+520*Math.pow(power,2.3)+(this.random()-.5)*22,
        releaseVY:-60-250*power+(this.random()-.5)*18,trace:[]});this.shots++;}
  ...
  const api={World,STEP,WIDTH,HEIGHT,RADIUS,pins,groups,walls,rail,railPoint,railLength,stage,stageY,rightGate};
