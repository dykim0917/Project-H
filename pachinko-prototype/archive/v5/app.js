...
function render(){
  ...
  $('power-value').textContent=percent.toFixed(1)+'%';$('power').value=String(percent);
...
$('power-minus').onclick=()=>setPower(world.power*100-.5);$('power-plus').onclick=()=>setPower(world.power*100+.5);
...
$('sound').onclick=()=>{unlockAudio();muted=!muted;soundscape?.mute(muted);$('sound').textContent='소리 '+(muted?'OFF':'ON');};
$('mechanical-volume').oninput=e=>{mechanicalLevel=Number(e.target.value)/100;soundscape?.setLevels(mechanicalLevel,cueLevel);};
$('cue-volume').oninput=e=>{cueLevel=Number(e.target.value)/100;soundscape?.setLevels(mechanicalLevel,cueLevel);};
$('speed').onclick=()=>{speed=speed===1?3:1;$('speed').textContent='전체 속도 ×'+speed;for(const track of document.querySelectorAll('.reel-track'))track.style.animationDuration=(track.parentElement=== $('r1')&&$('caption').textContent==='착륙 시도…'?.85:.30)/speed+'s';};
...
document.addEventListener('visibilitychange',()=>{lastFrame=null;accumulator=0;if(document.hidden)soundscape?.rolling(0);});
